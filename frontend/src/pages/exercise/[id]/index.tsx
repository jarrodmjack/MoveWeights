...
const index = () => {
	...
	const handleDeleteSetFromExercise = useCallback(
		async (setId: string) => {
			try {
				const response = await fetch(
	...
	return (
		<Layout>
			<div className="flex flex-col">
				<div className="flex justify-center self-center w-full bg-info-content">
					<span
						onClick={() => setActiveTab(ActiveTab.EXERCISE)}
						className={`text-xl text-white py-1 w-1/2 text-center ${
							activeTab === ActiveTab.EXERCISE &&
